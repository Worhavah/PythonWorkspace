...
payload = {'q': '燕窝','s': '1','ie':'utf8'}  #字典传递url参数
file = open('taobao_test21.txt','w',encoding='utf-8')

for k in range(0,5):        #100次，就是100个页的商品数据
    ...
    nicks = re.findall(r'"nick":"([^"]+)"', resp.text, re.I)
    x = len(title)           #每一页商品的数量

    for i in range(0,x) :    #把列表的数据保存到文件中
        file.write(str(k*44+i+1)+'商品：'+title[i]+'\n'+'价格：'+price[i]+'\n'+'地址：'+'\n'+'nicks：'+nicks[i]+'\n\n')
